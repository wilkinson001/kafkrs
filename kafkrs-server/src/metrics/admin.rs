//! Admin-port HTTP listeners: `/metrics`, `/health`, `/ready`.
...
/// `ports.health`:
///
/// - Neither set → no listeners. No-op.
...
///
/// `prom_handle` is the Prometheus handle to serve on `/metrics`, or `None`
/// when the recorder wasn't installed (i.e. `ports.metrics` was unset).
pub(super) fn install(
    ports: &PortsConfig,
    prom_handle: Option<metrics_exporter_prometheus::PrometheusHandle>,
) -> anyhow::Result<()> {
